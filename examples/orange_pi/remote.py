...
Usage:
    ...
    python3 gamepad_driver.py <robot_ip> --stick right
...
def control_loop(sock, args):
    ...
    while not stop_flag.is_set():
        ...
        if btn_b or (x == 0.0 and y == 0.0):
            ...
            DIAG = 0.4
            # Определяем направление без учёта амплитуды
            if abs(y) >= DIAG and abs(x) >= DIAG:
                # диагональ → кривая
                if y > 0:
                    cmd = "CURVE_FR" if x > 0 else "CURVE_FL"
                else:
                    cmd = "CURVE_BR" if x > 0 else "CURVE_BL"
            elif abs(y) >= abs(x):
                # преимущественно вертикаль
                cmd = "FORWARD" if y > 0 else "BACKWARD"
            else:
                # поворот на месте
                cmd = "RIGHT" if x > 0 else "LEFT"
            cmd += " 0"      # <-- непрерывное движение
